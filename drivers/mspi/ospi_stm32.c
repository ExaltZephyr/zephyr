...
static bool mspi_stm32_is_memorymap(const struct device *dev)
{
	...
	return ((READ_BIT(dev_data->hmspi.Instance->CR,
...
static int mspi_stm32_memmap_off(const struct device *controller)
{
	...
	if (HAL_OSPI_Abort(&dev_data->hmspi) != HAL_OK) {
...
static int mspi_stm32_memmap_on(const struct device *controller)
{
	...
	if (ret != HAL_OK) {
		LOG_ERR("Failed to set memory map %d", dev_data->hmspi.ErrorCode);
		return -EIO;
	}
	...
	ret = HAL_OSPI_MemoryMapped(&dev_data->hmspi, &s_MemMappedCfg);
	if (ret != HAL_OK) {
			     ...
			     uint8_t access_mode)
{
	...
	if (packet->dir == MSPI_RX) {	
		...
		case MSPI_ACCESS_SYNC:
			hal_ret = HAL_OSPI_Receive(&dev_data->hmspi, packet->data_buf,
						   HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
			goto e_access;
		case MSPI_ACCESS_ASYNC:
			hal_ret = HAL_OSPI_Receive_IT(&dev_data->hmspi, packet->data_buf);
			break;
        case MSPI_ACCESS_DMA:
            hal_ret = HAL_OSPI_Receive_DMA(&dev_data->hmspi, packet->data_buf);
            break;    
		default:
			/* Not correct */
			hal_ret = HAL_BUSY;
		...
		/* Transmit the data */
		switch (access_mode) {
			LOG_INF("tend to write");
		case MSPI_ACCESS_SYNC:
			hal_ret = HAL_OSPI_Transmit(&dev_data->hmspi, packet->data_buf,
						    HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
			goto e_access;
		case MSPI_ACCESS_ASYNC:
					...
					uint8_t match_mask, uint32_t timeout_ms)
{
	...
	if (HAL_OSPI_AutoPolling_IT(&dev_data->hmspi, &s_config) != HAL_OK) {
...
static int mspi_stm32_mem_ready(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	if (HAL_OSPI_Command(&dev_data->hmspi, &s_command, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) !=
	    HAL_OK) {
					  ...
					  uint8_t cfg_rate)
{
	...
	if (HAL_OSPI_Transmit(&dev_data->hmspi, &transmit_data, HAL_OSPI_TIMEOUT_DEFAULT_VALUE ) !=
	    HAL_OK) {
				       ...
				       uint8_t op_enable)
{
	...
	if (HAL_OSPI_Transmit(&dev_data->hmspi, &op_enable, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) !=
	    HAL_OK) {
		LOG_ERR("Write Flash configuration reg2 failed");
		return -EIO;
	}
				   ...
				   uint8_t *value)
{
	...
	if (HAL_OSPI_Receive(&dev_data->hmspi, value, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
		LOG_ERR("Write Flash configuration reg2 failed");
		return -EIO;
	}
...
static int mspi_stm32_config_mem(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	dev_data->hmspi.Init.MemoryType = HAL_OSPI_MEMTYPE_MACRONIX  ;
	dev_data->hmspi.Init.DelayHoldQuarterCycle = HAL_OSPI_DHQC_ENABLE ;
	if (HAL_OSPI_Init(&dev_data->hmspi) != HAL_OK) {
...
static void mspi_stm32_isr(const struct device *dev)
{
	...
	HAL_OSPI_IRQHandler(&dev_data->hmspi);
}
 ...
 */
void HAL_OSPI_ErrorCallback(OSPI_HandleTypeDef *hmspi)
{
...
static int mspi_stm32_config(const struct mspi_dt_spec *spec)
{
	...
	if (dev_data->dev_cfg.data_rate == MSPI_DATA_RATE_DUAL) {
		...
		dev_data->hmspi.Init.MemoryType = HAL_OSPI_MEMTYPE_MICRON;
		dev_data->hmspi.Init.DelayHoldQuarterCycle = HAL_OSPI_DHQC_DISABLE;
	}
#if MSPI_STM32_DLYB_BYPASSED
	dev_data->hmspi.Init.DelayBlockBypass = HAL_OSPI_DELAY_BLOCK_BYPASSED;
#else
	dev_data->hmspi.Init.DelayBlockBypass = HAL_OSPI_DELAY_BLOCK_USED;
	...
	// #if defined(DLYB_XSPI1) || defined(DLYB_XSPI2) || defined(DLYB_OCTOSPI1) ||
